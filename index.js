...
const cloneRepo = async () => {
    try {
        ...
        console.error('Normal clone failed, retrying with --depth=1');
        try {
            // Retry with --depth=1
            await runCommand(`git clone --depth=1 ${sourceRepo}`, 'Step 1 (retry): Repository cloned successfully with --depth=1', 'Step 1 (retry): Failed to clone repository with --depth=1');
        } catch (retryError) {
